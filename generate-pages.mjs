...
const SITE = "https://priyanka-eg.shop";
...
function orgGraph() {
  return [
    ...
    },
    {
      "@type": "ImageObject",
      "@id": `${SITE}/#logo`,
      contentUrl: `${SITE}/images/logo.jpg`,
...
function productNode(item) {
  return {
    ...
    image: item.img.startsWith("http") ? item.img : `${SITE}${item.img}`,
...
const home = `${head({
  ...
  graph: [
    ...
    itemList(`${SITE}/#featured`, "منتجات مميزة من بريانكا للتجميل", featured),
  ],
})}
  ${splash}
...
const productsPage = `${head({
  ...
  graph: [
    ...
    itemList(`${SITE}/#catalog`, "كتالوج بريانكا للتجميل", products),
  ],
})}
  <a class="skip-link" href="#main">تخطي إلى المحتوى</a>
...
const contactPage = `${head({
  ...
  graph: [
    ...
    {
      "@type": "HowTo",
      "@id": `${SITE}/contact.html#howto-order`,
